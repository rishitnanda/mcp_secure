...
- **Vulnerability to Advanced Indirect Prompt Injections (e.g., AIShellJack)** — The current defenses rely on a static regex blacklist and simple substring-based output sanitization. These can be bypassed by sophisticated adversarial prompts (such as those tested in the AIShellJack framework) that utilize obfuscated command execution, multi-stage payloads, and untrusted workspace resource poisoning (e.g., `.cursorrules` or `README.md`). Fundamentally, the suite intercepts at the MCP protocol boundary only. Any injection that reaches the LLM context through a channel other than a tool call — system prompt manipulation, direct resource reads, or multi-modal inputs — is outside the scope of this defence.
